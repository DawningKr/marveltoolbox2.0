...
class BaseTrainer:
    def __init__(self, config_path="./trainer.yaml"):
        ...
        utils.check_configurations(
            configs, {"batch_size": None, "seed": None, "paths": ["log", "checkpoint"]}
        )
    ...
    def save(self, is_best=False):
        ...
        utils.save_checkpoint(
            state_dict,
            is_best,
            file_path=self.configs["paths"]["checkpoint"],
            flag=self.save_flag,
        )

    def load(self, flag, is_best=False):
        checkpoint = utils.load_checkpoint(
            is_best, file_path=self.configs["paths"]["checkpoint"], flag=flag
        )
